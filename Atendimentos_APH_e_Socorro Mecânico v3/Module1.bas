...
'Purge data of service time smaller than minimum time
...
'Expurga dados com tempo de atendimento abaixo de um tempo mínimo

'Version: 3.0

'Created by Matheus Nunes Reis on 04/11/2024
...
'''----------
    ...
    With ObjWorksheet.Sort
        ...
        .SetRange ObjWorksheet.Range("A1:O" & LastRowObjWorksheet) 'Ordena a tabela da coluna A até O
        .Header = xlYes
        .Apply
    End With
...
''''''' Etapa de, linha por linha, realizar análise do 1º atendimento, expurgar dados com tempos <=0, substitiur os recursos pelos tipos de veículos, verificar nem guincho nem ambulância,
    ...
    DestWorksheet.Range(DestWorksheet.Cells(1, "A"), DestWorksheet.Cells(1, "M")).Value = ObjWorksheet.Range(ObjWorksheet.Cells(1, "A"), ObjWorksheet.Cells(1, "M")).Value
    DestWorksheet.Range("N1").Value = "t. Ocorrência"
    DestWorksheet.Range("O1").Value = "t. Acionamento"
    DestWorksheet.Range("P1").Value = "Mês"
    DestWorksheet.Range("Q1").Value = "Tempo"
    ...
    For i = 2 To LastRowObjWorksheet 'i é linha na ObjWorksheet
        ...
        Base = ObjWorksheet.Cells(i, "M").Value
        ...
        ElseIf PrimeiroAtend = 0 And t_ocorrencia = 0 And Base = 2 Then 'PrimeiroAtend = 0 relacion-ase ao t_ocorrencia
            t_ocorrenciaZero = t_ocorrenciaZero + 1
            QtdeExpurgo = QtdeExpurgo + 1
            PrimeiroAtend = 1
        ElseIf PrimeiroAtend = 1 And t_acionamento = 0 And Base = 2 Then 'PrimeiroAtend = 1 relaciona-se ao t_acionamento
        ...
        Else
            ...
            DestWorksheet.Range(DestWorksheet.Cells(LinhaDestworksheet, "A"), DestWorksheet.Cells(LinhaDestworksheet, "M")).Value = ObjWorksheet.Range(ObjWorksheet.Cells(i, "A"), ObjWorksheet.Cells(i, "M")).Value
            
            'Aplica informações das colunas M, N e O na planilha destino
            DestWorksheet.Cells(LinhaDestworksheet, "N").Value = t_ocorrencia
            DestWorksheet.Cells(LinhaDestworksheet, "O").Value = t_acionamento
            DestWorksheet.Cells(LinhaDestworksheet, "P").Value = Format(DestWorksheet.Cells(LinhaDestworksheet, "D").Value, "mm") 'Cria coluna com número do mês. OBS: esta coluna é apagada ao final do processamento
            DestWorksheet.Cells(LinhaDestworksheet, "Q").Value = Tempo
 ...
 '''''''
    ...
    ' Classificar pelas colunas E (Serviço), F (Recurso), P (Mês) e Q (Tempo)
    ...
    With DestWorksheet.Sort
        ...
        .SortFields.Add Key:=DestWorksheet.Range("Q1:Q" & LastRowDestWorksheet), Order:=xlAscending
        .SetRange DestWorksheet.Range("A1:Q" & LastRowDestWorksheet) 'Ordena a tabela da coluna A até Q
        .Header = xlYes
        .Apply
    End With
    ...
    DadosExpWorksheet.Range("K1").Value = "Tempo Reduzido"
    DadosExpWorksheet.Range("K2").Value = TempoReduzido
