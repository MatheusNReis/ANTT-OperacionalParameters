...
'Created by Matheus Nunes Reis on 08/11/2024
...
''''''' Etapa de, linha por linha, realizar análise do 1º atendimento, expurgar dados com tempos <=0, substitiur os recursos pelos tipos de veículos, verificar nem guincho nem ambulância,
    ...
    DestWorksheet.Range(DestWorksheet.Cells(1, "A"), DestWorksheet.Cells(1, "O")).Value = ObjWorksheet.Range(ObjWorksheet.Cells(1, "A"), ObjWorksheet.Cells(1, "O")).Value
    DestWorksheet.Range("P1").Value = "t. Ocorrência"
    DestWorksheet.Range("Q1").Value = "t. Acionamento"
    DestWorksheet.Range("R1").Value = "Mês"
    DestWorksheet.Range("S1").Value = "Tempo"
    DestWorksheet.Range("T1").Value = "Ano"
    DestWorksheet.Range("U1").Value = "Rodovia Concat"
    DestWorksheet.Range("V1").Value = "Contagem"
    ...
    For i = 2 To LastRowObjWorksheet 'i é linha na ObjWorksheet
        ...
        Else
            ...
            'Aplica informações das colunas A:N na planilha destino usando Range
            DestWorksheet.Range(DestWorksheet.Cells(LinhaDestworksheet, "A"), DestWorksheet.Cells(LinhaDestworksheet, "O")).Value = ObjWorksheet.Range(ObjWorksheet.Cells(i, "A"), ObjWorksheet.Cells(i, "O")).Value
            
            'Aplica informações das colunas P, Q, R, S e T na planilha destino
            DestWorksheet.Cells(LinhaDestworksheet, "P").Value = t_ocorrencia
            DestWorksheet.Cells(LinhaDestworksheet, "Q").Value = t_acionamento
            DestWorksheet.Cells(LinhaDestworksheet, "R").Value = Format(DestWorksheet.Cells(LinhaDestworksheet, "D").Value, "mm")
            DestWorksheet.Cells(LinhaDestworksheet, "S").Value = Tempo
            DestWorksheet.Cells(LinhaDestworksheet, "T").Value = Format(DestWorksheet.Cells(LinhaDestworksheet, "D").Value, "yyyy")
 ...
 '''''''
    ...
    For j = 2 To LastRowDestWorksheet
        ...
        Else
            ...
            DestWorksheet.Cells(j, "V").Value = 1
